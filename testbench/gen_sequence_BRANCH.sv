    ...
    virtual task body();
        ...
        while (instruction_counter < 2**`MLEN/(4*2) ) begin
            ...
            if ( instruction_counter <= 2**`MLEN/(4*2) - 30) begin
                ...
                item_BRANCH.randomize() with {opcode==S_B_TYPE && funct3==BGE_FC && rs1!=0 && rs2!=0 && rs1!=rs2 && imm==(total_nops+1+1+1)*(4/2);};  //offset = nops + addi_rupt + jal + 1   //&& imm==(total_nops+1+1+1)*4
                ...
                // BLT
                end else if (item_BRANCH.funct3 == BLT_FC) begin
                ...
                // BGE
                end else if (item_BRANCH.funct3 == BGE_FC) begin
